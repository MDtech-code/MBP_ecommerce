// src/features/profile/ui/AvatarModal.jsx
//
// Changes:
//   - AlertBanner adopted for uploadError
//   - Dark mode added to modal, header, footer, buttons
//   - Dark mode on preview circle
...
export default function AvatarModal({ currentAvatar, onClose }) {
  ...
  const handleUpload = () => {
    ...
    uploadAvatar(formData, { onSuccess: () => onClose() })
  }
  ...
  return (
    <Portal>
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        {/* Modal */}
        <div className="relative z-10 bg-white dark:bg-[#0a0a0a] rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden border border-gray-100 dark:border-gray-800">
          ...
          <div className="px-6 py-6 flex flex-col items-center gap-4">
            <div className="w-32 h-32 rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden border-4 border-gray-200 dark:border-gray-700 flex items-center justify-center">
              {preview ? (
                <img src={preview} alt="Preview" className="w-full h-full object-cover" />
              ) : currentAvatar ? (
                <img src={getMediaUrl(currentAvatar)} alt="Current avatar" className="w-full h-full object-cover" />
              ) : (
                <UserRound size={60} className="text-gray-300 dark:text-gray-600" />
              )}
            </div>

            {/* ── OLD ──────────────────────────────────────────────────
            {uploadError && (
              <div role="alert" className="w-full text-sm text-red-600 bg-red-50
                border border-red-200 rounded-lg px-4 py-2 text-center">
                {uploadError}
              </div>
            )}
            ─────────────────────────────────────────────────────────── */}
            ...
            {uploadError && (
              <AlertBanner type="error" className="w-full">
                {uploadError}
              </AlertBanner>
            )}
            ...
            <button
              ...
              className="flex items-center gap-2 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 px-5 py-2.5 rounded-lg text-sm font-semibold hover:border-primary hover:text-primary dark:hover:border-primary dark:hover:text-primary transition-colors"
            >
              <Camera size={16} />
          ...
          {/* Footer */}
          <div className="flex gap-3 px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/40">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 py-2.5 rounded-lg text-sm font-bold hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
            >
              CANCEL
            </button>
            <button
