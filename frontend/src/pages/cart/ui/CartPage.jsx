 ...
 *
 * Phase 3 additions:
 *   - useCheckoutStore for item selection state
 *   - Selection props wired to CartList
 *   - Coupon + selection props wired to CartSummary
 *   - handleProceedToCheckout → syncs store → navigates to /checkout/address
 *
 * YouMayAlsoLike:
...
export default function CartPage() {
  ...
  // ── Checkout store — item selection ───────────────────────────────────────
  const selectedItemIds  = useCheckoutStore((s) => s.selectedItemIds);
  const selectedItems    = useCheckoutStore((s) => s.selectedItems);
  const toggleItem       = useCheckoutStore((s) => s.toggleItem);
  const selectAll        = useCheckoutStore((s) => s.selectAll);
  const clearSelection   = useCheckoutStore((s) => s.clearSelection);
  const setSelectedItems = useCheckoutStore((s) => s.setSelectedItems);
  ...
  const handleProceedToCheckout = () => {
    // Re-filter items to ensure selectedItems objects match selectedItemIds
    // (toggleItem keeps them in sync but this is a safety sync before navigate)
              ...
              <div className="lg:col-span-3 bg-white rounded-xl border border-gray-200 p-6 animate-pulse">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="flex gap-4 py-5 border-b border-gray-100">
                    <div className="w-4 h-4 bg-gray-100 rounded mt-1 shrink-0" />
