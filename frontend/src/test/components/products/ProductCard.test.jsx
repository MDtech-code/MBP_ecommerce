...
/**
 * useCart is mocked so ProductCard tests are isolated.
 * We control handleAddToCart and isMutating per test.
 */
...
beforeEach(() => {
  vi.clearAllMocks();
  mockIsMutating = false; // reset between tests
});
...
describe("ProductCard", () => {
  ...
  it("shows discount badge when has_discount is true", () => {
    renderCard(makeProduct({ has_discount: true, discount_percentage: 18 }));
  ...
  it("calls handleAddToCart with product.id and quantity 1 on click", () => {
    renderCard(makeProduct({ id: 1 }));
    fireEvent.click(screen.getByRole("button", { name: /add to cart/i }));
    expect(mockHandleAddToCart).toHaveBeenCalledWith(1, 1);
  });

  it("calls handleAddToCart exactly once per click", () => {
    renderCard(makeProduct());
    fireEvent.click(screen.getByRole("button", { name: /add to cart/i }));
    expect(mockHandleAddToCart).toHaveBeenCalledTimes(1);
  });

  it("does not call handleAddToCart when out of stock button clicked", () => {
    renderCard(makeProduct({ is_in_stock: false }));
    fireEvent.click(screen.getByRole("button", { name: /out of stock/i }));
    expect(mockHandleAddToCart).not.toHaveBeenCalled();
  });

  it("disables Add to Cart button when isMutating is true", () => {
    mockIsMutating = true;
    renderCard(makeProduct());
    expect(
      screen.getByRole("button", { name: /add to cart/i }),
  ...
  it("does not call handleAddToCart when isMutating is true", () => {
    mockIsMutating = true;
    renderCard(makeProduct());
    fireEvent.click(screen.getByRole("button", { name: /add to cart/i }));
    expect(mockHandleAddToCart).not.toHaveBeenCalled();
...
//   it("calls handleAddToCart with product.id and quantity 1", () => {
//     renderCard();

//     fireEvent.click(screen.getByRole("button", { name: /add to cart/i }));

//     expect(mockHandleAddToCart).toHaveBeenCalledWith(42, 1);
//   });
...
//   it("button is disabled when is_in_stock is false", () => {
//     renderCard({ is_in_stock: false });

//     const btn = screen.getByRole("button", { name: /out of stock/i });
//     expect(btn).toBeDisabled();
//   });

//   it("button is disabled when isMutating is true", () => {
//     mockIsMutating = true;
//     renderCard();
...
//   it("does not call handleAddToCart when button is disabled (out of stock)", () => {
//     renderCard({ is_in_stock: false });
...
//   it("does not call handleAddToCart when button is disabled (isMutating)", () => {
//     mockIsMutating = true;
//     renderCard();
