...
vi.mock("../../../api/auth", () => ({
  hasAuthToken, // ← hoisted reference
}));
...
// ─────────────────────────────────────────────────────────────────────────────
// Mock ProfileView — captures props via spy
// ─────────────────────────────────────────────────────────────────────────────
...
vi.mock(
  ...
  () => ({
    default: (props) => {
      profileView(props);
      return (
        <div data-testid="profile-view">Profile View</div>
      );
    },
  }),
);
...
vi.mock(
  ...
  () => ({
    default: () => (
      <div data-testid="profile-edit">Edit Form</div>
    ),
  }),
);
...
vi.mock(
  ...
  () => ({
    default: () => (
      <div data-testid="avatar-modal">Avatar Modal</div>
    ),
  }),
);
...
beforeEach(() => {
  ...
  // hasAuthToken returns true by default so enabled = true
...
describe("Profile Page", () => {
  ...
  it("passes onAvatarClick callback to ProfileView", () => {
    ...
    expect(
      typeof profileView.mock.calls[0][0].onAvatarClick,
    ).toBe("function");
  });

  it("passes avatar to AvatarModal after callback", async () => {
    render(<Profile />);

    const callback =
      profileView.mock.calls[0][0].onAvatarClick;

    // Wrap state-updating callback in act so React
    // flushes the re-render before we assert
    await act(async () => {
      callback();
    });
