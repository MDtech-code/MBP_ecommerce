...
def list_shipments(
    *,
    ...
    """
    Return a filtered QuerySet of all shipments for admin listing.

    All filters are optional — passing None omits that filter.
    Returns lazy QuerySet — view paginates.

    Args:
        status:  Filter by Shipment.Status value.
        courier: Filter by CourierPartner value.
        city:    Filter by destination_city (case-insensitive).

    Returns:
        Lazy QuerySet of Shipment instances ordered by -created_at.
    """
    qs = (
        Shipment.objects
        .select_related(
    ...
    if city:
        qs = qs.filter(destination_city__iexact=city)
