...
# ─────────────────────────────────────────────────────────────────────────────
# NEW SELECTORS — added for dashboard reviews feature
# ─────────────────────────────────────────────────────────────────────────────

def get_all_reviewable_items(user) -> QuerySet:
    """
    Return ALL OrderItems the user can review across all delivered orders.

    Unlike get_reviewable_items() which scopes to one product,
    this returns every unreviewed eligible item across all products.
    Used by the dashboard Pending Reviews section.

    Eligibility rules:
        1. OrderItem belongs to requesting user.
        2. Order status is DELIVERED.
        3. No review submitted yet for this OrderItem.

    Prefetches product images for dashboard card display.
    select_related order to access order_number without N+1.

    Args:
        user: Authenticated user requesting the list.

    Returns:
        QuerySet of OrderItem instances ordered by most recently
        delivered first.
    """
    return (
        OrderItem.objects
...
def get_reviews_by_user(user) -> QuerySet:
    """
    Return all reviews submitted by this user, newest first.

    Includes both approved and pending moderation reviews.
    User is entitled to see their own review status.

    Used by dashboard Review History section.

    Prefetches product images for dashboard card display.
    select_related order_item__order for order_number access.

    Args:
        user: Authenticated user requesting their review history.

    Returns:
        QuerySet of Review instances ordered by newest first.
    """
    return (
        Review.objects
