...
#     Design decisions:
#     - compatible_bikes M2M → core "fits my bike" feature.
#     - weight_grams on model for programmatic shipping calculation.
#       Also stored in ProductSpecification for human display.
#     - low_stock_threshold triggers LowStockAlert via post_save signal.
#     - All price properties computed — Decimal precision preserved.
#     - status + stock together determine is_in_stock.
#     """
...
#     models.Index(fields=["brand",      "status"]),
#     models.Index(fields=["is_featured","status"]),
#     models.Index(fields=["stock",      "status"]),  # low stock queries
# ]
#     def __str__(self) -> str:
#         return self.name
...
#         """
#         True when stock is at or below threshold and threshold > 0.
#         Used by signal to decide whether to create LowStockAlert.
#         """
#         return (
#             self.low_stock_threshold > 0
#             and self.stock <= self.low_stock_threshold
#         )
...
#         Uses select_for_update() to prevent concurrent uploads
#         both being marked primary simultaneously.
#         """
#         if self.is_primary:
#             self._set_as_sole_primary(*args, **kwargs)
#         else:
...
#     def _set_as_sole_primary(self, *args, **kwargs) -> None:
#         """
#         Atomically sets this image as the sole primary image.
#         Mirrors UserAddress._set_as_sole_default() pattern.
#         """
#         with transaction.atomic():
#             # Force QuerySet evaluation → SELECT FOR UPDATE fires
#             list(
#                 ProductImage.objects.select_for_update().filter(
#                     product_id=self.product_id
#                 )
#             )
#             # Safe to update — we hold row locks on all images for this product
#             ProductImage.objects.filter(
#                 product_id=self.product_id,
#                 is_primary=True,
#             ).exclude(pk=self.pk).update(is_primary=False)
...
#     Prevents overselling when multiple customers checkout same
#     product simultaneously — critical during Eid/flash sale traffic.
...
#         StockReservation.objects.filter(expires_at__lt=now).delete()
#         (signals restore stock on each deleted reservation)

#     unique_together: one reservation per user per product —
#     prevents same user double-reserving same product.
#     """
...
#             self.expires_at = (
#                 timezone.now()
#                 + timedelta(minutes=self.RESERVATION_MINUTES)
#             )
#         super().save(*args, **kwargs)
#         logger.debug(
#             "StockReservation: product=%s user=%s qty=%s expires=%s",
...
#     def __str__(self) -> str:
#         return (
#             f"Reserve {self.quantity}x {self.product.name} "
#             f"for {self.user.email} until {self.expires_at}"
#         )
...
#     Partial immutability:
#     - product_id, stock_at_alert, threshold_at_alert → immutable
#     - is_resolved, resolved_at → mutable (resolution workflow)
#     """
...
#     def save(self, *args, **kwargs) -> None:
#         if self.pk:
#             # Enforce partial immutability — only resolution fields mutable
#             original = LowStockAlert.objects.get(pk=self.pk)
#             immutable = ["product_id", "stock_at_alert", "threshold_at_alert"]
#             for field in immutable:
#                 if getattr(self, field) != getattr(original, field):
#                     raise ValueError(
#                         f"LowStockAlert.{field} is immutable after creation."
#                     )
