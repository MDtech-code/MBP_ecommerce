...
if (response.data.success) {
    ...
    // Non-field errors — show as toast / banner
    if (errors.non_fields) {
        const { category, message, code } = errors.non_fields

        if (category === "domain") {
            // Business rule — show message to user
            showToast(message)
        }

        if (category === "system") {
            // Infrastructure — show retry message
            showToast("Service temporarily unavailable. Please try again.")
        }

        if (category === "validation") {
            // Auth / permission — redirect or show banner
            showBanner(message)
        }
    }
}
...
```
1. exc.internal is NEVER serialized into any response
   Only written to logs and Sentry

2. In production — unexpected errors show generic message only
   Raw exception messages never reach the client

3. Refresh token never appears in response body
   Only set as HttpOnly cookie

4. One log entry per exception — no duplicate logging
   All logging in custom_exception_handler only
   Builders are pure functions with zero side effects
```